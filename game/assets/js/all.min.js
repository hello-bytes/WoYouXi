...
function addToOnloadFunc(func){var oldonload=window.onload;if(typeof window.onload!='function'){window.onload=func;}else{window.onload=function(){oldonload();func();}}}
function smartLoadImage()
{var offsetPage=window.pageYOffset?window.pageYOffset:window.document.documentElement.scrollTop,offsetWindow=offsetPage+Number(window.innerHeight?window.innerHeight:document.documentElement.clientHeight),docImg=document.images,_len=docImg.length;if(!_len)return false;for(var i=0;i<_len;i++){var attrSrc=docImg[i].getAttribute("data"),o=docImg[i],tag=o.nodeName.toLowerCase();if(o){postPage=o.getBoundingClientRect().top+window.document.documentElement.scrollTop+window.document.body.scrollTop;postWindow=postPage+Number(this.getStyle(o,'height').replace('px',''));if((postPage>offsetPage&&postPage<offsetWindow)||(postWindow>offsetPage&&postWindow<offsetWindow)){if(tag==="img"&&attrSrc!==null){o.setAttribute("src",attrSrc);}
o=null;}}}}
...
var imgLoaderFunc=function(){};function attachImageError(){docImg=document.images;if(docImg!=null){_len=docImg.length;for(var i=0;i<_len;i++){if(docImg[i].onerror==null&&docImg[i].getAttribute("errorimg")!==null){docImg[i].onerror=function(e){var img=null;if(e!=null){img=e.target;}else if(event!=null){img=event.srcElement;}
if(img!=null){var errorImg=img.getAttribute("errorimg");img.onerror=null;if(errorImg!==null){img.setAttribute("src",errorImg);}}}}else{}}}}
attachImageError();if(YS===undefined){var YS={};(function(Y){function mix(s,o,w,pro){var t1,t2,a;if(s===undefined||o===undefined)return{};w=w||0;for(a in o){t1=s[a];t2=o[a];if(typeof t2=='object'){typeof t1=='object'?mix(t1,t2,w,pro):s[a]=t2;}
if(w||!s.hasOwnProperty(a)||!t1)s[a]=t2;}}
function imgLoadLater(o){var self=this;if(!(self instanceof imgLoadLater)){return new imgLoadLater(o);}
config=self._config;mix(config,o,true);config.screenH=self._getScreen();self._config=config;self._init();}
mix(imgLoadLater.prototype,{_config:{dataSrc:'data',screenValue:1.5},_getTop:function(elem){return elem.getBoundingClientRect().top;},_imgLoad:function(){var self=this,config=self._config,imgs=config.imgs||[],tops=config.tops||[],dataSrc=config.dataSrc,l=imgs.length,winH=config.screenH+self._getScrollTop(),arrImgs=[],arrTops=[],attr,top,img;if(l<1)return;while(l--){top=tops[l];img=imgs[l];if(tops[l]<=winH){attr=img.getAttribute(dataSrc);attr&&(img.src=attr);}else{arrImgs.push(img);arrTops.push(top);}}
self._config.imgs=arrImgs;self._config.tops=arrTops;},_getScreen:function(){return document.documentElement.clientHeight*this._config.screenValue;},_getScrollTop:function(){　　　　return Math.max(document.body.scrollTop,document.documentElement.scrollTop);},_filterImg:function(){var self=this,config=self._config,tops=[],imgs=config.imgs||[],dataSrc=config.dataSrc,placehold=config.placehold,scrollTop=self._getScrollTop(),winH=config.screenH+scrollTop,arrImgs=imgs.length?imgs:document.getElementsByTagName('img'),len=arrImgs.length,tmp;imgs=[];while(len--){tmp=arrImgs[len];if(tmp.getAttribute(dataSrc)){tops.push(self._getTop(tmp)+scrollTop);imgs.push(tmp);if(placehold){tmp.src=placehold;}}}
self._config.imgs=imgs;self._config.tops=tops;self._filterImg=function(){};},_addEvent:function(elem,type,fn,b){if(elem.addEventListener){elem.addEventListener(type,fn,b||false);}else if(elem.attachEvent){elem.attachEvent('on'+type,fn);}else{elem['on'+type]=fn;}},_removeEvent:function(elem,type,fn){if(elem.removeEventListener){elem.removeEventListener(type,fn);}else if(elem.detachEvent){elem.detachEvent('on'+type,fn);}},_init:function(){var self=this;self._filterImg();self._addEvent(window,'scroll',loader);self._addEvent(window,'resize',(resizeLoader=function(){self._config.screenH=self._getScreen();setTimeout(function(){self._imgLoad();},100);}));function loader(){self._imgLoad();if(self._config.imgs.length==0){self._removeEvent(window,'scroll',loader);self._removeEvent(window,'resize',resizeLoader);}}
loader();}});Y._mix=mix;Y.imgLoadLazy=imgLoadLater;Y.loadImgs=function(imgs,attr){var len=typeof imgs==='object'&&imgs.length,tmp,tmpAttr;if(len){attr=attr||'data';while(len--){tmp=imgs[len],tmpAttr=tmp.getAttribute(attr);if(tmpAttr){tmp.setAttribute('src')=tmpAttr;tmp.removeAttribute(attr);}else{return true;}}}
return false;}})(YS);}
YS.imgLoadLazy();function slideToLeft(){var obj=document.getElementById("slideul");if(obj!=null){if(obj.style.marginLeft=="-495px"){obj.style.marginLeft="0px"}else{obj.style.marginLeft="-495px";}}}
function slideToRight(){var obj=document.getElementById("slideul");if(obj!=null){if(obj.style.marginLeft=="-495px"){obj.style.marginLeft="0px";}else{obj.style.marginLeft="-495px";}}}
function tips_pop(elementId){var MsgPop=document.getElementById(elementId);if(MsgPop==null)return;var popH=parseInt(MsgPop.style.height);if(popH==0){MsgPop.style.display="block";show=setInterval("changeH('up')",2);}else{hide=setInterval("changeH('down')",2);}}
function changeH(str){var MsgPop=document.getElementById("gameintropopup");var popH=parseInt(MsgPop.style.height);if(str=="up"){if(popH<=220){MsgPop.style.height=(popH+4).toString()+"px";}else{clearInterval(show);}}
if(str=="down"){if(popH>=4){MsgPop.style.height=(popH-4).toString()+"px";}else{clearInterval(hide);MsgPop.style.display="none";}}}
function shownewhotgame(){var newhotgame=getElementEx("newhotgamelist");var newgame=getElementEx("newgamelist");var newgametab=getElementEx("hotgamelisttab");var newhotgametab=getElementEx("newhotgamelisttab");if(newgametab!=null){newgametab.className="catalogtab";}
if(newhotgametab!=null){newhotgametab.className="catalogtab_select";}
if(newhotgame!=null){newhotgame.style.display="block";}
if(newgame!=null){newgame.style.display="none";}}
function shownewgame(){var newhotgame=getElementEx("newhotgamelist");var newgame=getElementEx("newgamelist");if(newhotgame!=null){newhotgame.style.display="none";}
if(newgame!=null){newgame.style.display="block";}
var newgametab=getElementEx("hotgamelisttab");var newhotgametab=getElementEx("newhotgamelisttab");if(newgametab!=null){newgametab.className="catalogtab_select";}
if(newhotgametab!=null){newhotgametab.className="catalogtab";}}
