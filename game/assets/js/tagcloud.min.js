var radius=100;var dtr=Math.PI/180;var d=207;var mcList=[];var active=false;var lasta=10;var lastb=10;var distr=true;var tspeed=2;var size=200;var mouseX=0;var mouseY=0;var howElliptical=1;var aA=null;var oDiv=null;var tagCloundInit=function()
{var i=0;var oTag=null;oDiv=document.getElementById('cloundtag_div');if(oDiv==null)return;aA=oDiv.getElementsByTagName('a');for(i=0;i<aA.length;i++)
...
{var oEvent=window.event||ev;mouseX=oEvent.clientX-(oDiv.offsetLeft+oDiv.offsetWidth/2);mouseY=oEvent.clientY-(oDiv.offsetTop+oDiv.offsetHeight/2);mouseX/=5;mouseY/=5;};setInterval(update,100);};addToOnloadFuncEx(tagCloundInit);function update()
...
{aA[i].style.left=mcList[i].cx+l-mcList[i].offsetWidth/2+'px';aA[i].style.top=mcList[i].cy+t-mcList[i].offsetHeight/2+'px';aA[i].style.fontSize=Math.ceil(12*mcList[i].scale/2)+7+'px';aA[i].style.filter="alpha(opacity="+100*mcList[i].alpha+")";aA[i].style.opacity=mcList[i].alpha;}}
